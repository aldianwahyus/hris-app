...
@section('judul', 'Riwayat Dokumen Saya')
...
.status.siap{background:var(--hijau-muda);color:var(--hijau-tua)}
...
<div class="kepala">
  <div>
    ...
    <p>Seluruh permintaan dokumen mandiri yang pernah Anda ajukan</p>
  </div>
  <a href="{{ route('documents.create') }}" class="btn" style="padding:8px 14px">Ajukan Dokumen</a>
</div>
  ...
  @forelse ($requests as $r)
    <div class="baris">
      <div class="baris-atas">
        <div>
          <div class="j">{{ $documentTypes[$r->document_type] ?? $r->document_type }}</div>
          <div class="s">{{ $r->purpose }}</div>
          <div class="s angka">Diajukan {{ date('j M Y', strtotime($r->created_at)) }}</div>
        </div>
        <span class="status {{ $r->status }}">
          {{ ['pending' => 'Menunggu', 'siap' => 'Siap Diunduh', 'ditolak' => 'Ditolak'][$r->status] ?? $r->status }}
        </span>
      </div>

      @if ($r->status === 'ditolak' && ! empty($r->decision_note))
        <div class="alasan">Alasan penolakan: {{ $r->decision_note }}</div>
      @endif

      @if ($r->status === 'siap')
        <div class="aksi-baris">
          <a href="{{ route('documents.download', $r->id) }}" class="tautan">Unduh PDF</a>
        </div>
      @endif
    </div>
  @empty
    <div class="kosong">Belum ada permintaan dokumen.</div>
  @endforelse
</div>
