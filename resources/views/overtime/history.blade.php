...
@section('judul', 'Riwayat Lembur Saya')
...
.aksi-baris{margin-top:10px;display:flex;gap:8px}
...
<div class="kepala">
  <div>
    ...
    <p>Seluruh pengajuan lembur yang pernah Anda kirim</p>
  </div>
  <a href="{{ route('overtime.create') }}" class="btn" style="padding:8px 14px">Ajukan Lembur</a>
</div>
  ...
  @forelse ($requests as $r)
    <div class="baris">
      <div class="baris-atas">
        <div>
          <div class="j">{{ $r->overtime_type === 'crash_program' ? 'Crash program' : ($r->overtime_type === 'shift_picket' ? 'Shift / Piket' : 'Lembur biasa') }} — {{ date('j M Y', strtotime($r->work_date)) }}</div>
          <div class="s angka">{{ rtrim(rtrim(number_format($r->payable_hours, 1, ',', '.'), '0'), ',') }} jam &middot; {{ $r->spkl_number }}</div>
        </div>
        <span class="status {{ $r->status }}">
          {{ ['pending' => 'Menunggu Atasan', 'pending_pimpinan' => 'Menunggu Pimpinan', 'approved' => 'Disetujui', 'rejected' => 'Ditolak', 'cancelled' => 'Dibatalkan', 'expired' => 'Kedaluwarsa', 'disbursed' => 'Sudah Dicairkan'][$r->status] ?? $r->status }}
        </span>
      </div>
      ...
      <div class="aksi-baris">
        @if (in_array($r->status, ['approved', 'disbursed'], true))
          <a href="{{ route('overtime.spkl', $r->id) }}" class="btn luar" style="padding:6px 12px">Cetak SPKL</a>
        @endif
        @if ($r->status === 'pending')
          <form method="POST" action="{{ route('overtime.cancel', $r->id) }}" onsubmit="return confirm('Batalkan pengajuan lembur ini?')">
            @csrf
