...
@section('judul', 'Riwayat SPPD Saya')
...
<div class="kepala">
  <div>
    <h2>Riwayat SPPD Saya</h2>
    <p>Seluruh pengajuan perjalanan dinas yang pernah Anda kirim</p>
  </div>
  <a href="{{ route('sppd.create') }}" class="btn" style="padding:8px 14px">Ajukan SPPD</a>
</div>
...
<div class="daftar">
  @forelse ($requests as $r)
    <div class="baris">
      <div class="baris-atas">
        <div>
          <div class="j">{{ $r->destination }}</div>
          <div class="s angka">{{ date('j M', strtotime($r->start_date)) }}–{{ date('j M Y', strtotime($r->end_date)) }} &middot; {{ $r->request_number }}</div>
        </div>
        <span class="status {{ $r->status }}">
          {{ ['pending' => 'Menunggu Atasan', 'pending_pimpinan' => 'Menunggu Pimpinan', 'approved' => 'Disetujui', 'rejected' => 'Ditolak', 'cancelled' => 'Dibatalkan', 'disbursed' => 'Sudah Dicairkan'][$r->status] ?? $r->status }}
        </span>
      </div>
      ...
      @if ($r->status === 'pending')
        <div class="aksi-baris">
          <form method="POST" action="{{ route('sppd.cancel', $r->id) }}" onsubmit="return confirm('Batalkan pengajuan SPPD ini?')">
            @csrf
  ...
  @empty
    <div class="kosong">Belum ada pengajuan SPPD.</div>
  @endforelse
</div>
