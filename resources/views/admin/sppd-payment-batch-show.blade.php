...
@section('judul', 'Batch Pembayaran SPPD Massal')
...
<div class="kepala">
  ...
  <p>
    {{ $batch->payer_scope === 'hc' ? 'Kantor Pusat' : $batch->office_name }}
    · Grup memo: {{ $batch->group_number }} · Penandatangan: {{ $batch->signatory_name }}
  </p>
</div>
...
<div class="grid">
  <div class="metrik">
    <div class="angka-besar">Rp {{ number_format($batch->total_amount_cents / 100, 0, ',', '.') }}</div>
    <div class="label">Total Dibayarkan</div>
  </div>
</div>

<div class="aksi">
  <button type="button" class="mini utama" onclick="bukaCetak('{{ route('admin.sppd-payment-batch.print-nota-debet', $batch->id) }}', 'Nota Debet')">Cetak Nota Debet</button>
  <button type="button" class="mini utama" onclick="bukaCetak('{{ route('admin.sppd-payment-batch.print-jurnal-slip', $batch->id) }}', 'Jurnal Slip')">Cetak Jurnal Slip</button>
</div>
...
<div class="gulir">
  <table>
    <thead>
      <tr><th>Pegawai</th><th>No. Rekening</th><th>Jumlah</th></tr>
    </thead>
    <tbody>
      @foreach ($items as $i)
        <tr>
          ...
          <td class="angka">Rp {{ number_format($i->amount_cents / 100, 0, ',', '.') }}</td>
        </tr>
      @endforeach
    </tbody>
  </table>
