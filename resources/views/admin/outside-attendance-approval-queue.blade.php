...
@section('judul', 'Antrean Persetujuan Absen Luar Kantor')
@section('peran', 'Pimpinan Kantor')
...
@section('isi')
<div class="kepala">
  <h2>Antrean persetujuan absen luar kantor</h2>
...
<div class="gulir">
  <table>
    <thead>
      <tr>
        <th>Pemohon</th><th>Tanggal</th><th>Alasan</th><th>Tindakan</th>
      </tr>
    </thead>
    <tbody>
      @forelse ($rows as $r)
        <tr>
          <td class="peg">{{ $r->requesting_name }}<small>{{ $r->nrp }}</small></td>
          ...
          <td>
            ...
            @else
              <div class="aksi">
                <form method="POST" action="{{ route('admin.outside-attendance-approve', $r->id) }}">
                  @csrf
                  <button class="mini utama" type="submit">Setujui</button>
                </form>
                <form method="POST" action="{{ route('admin.outside-attendance-reject', $r->id) }}">
                  @csrf
      ...
      @empty
        <tr>
          <td colspan="4" class="kosong">Tidak ada pengajuan yang menunggu keputusan.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
