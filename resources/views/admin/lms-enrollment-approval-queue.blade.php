...
@section('judul', 'Antrean Persetujuan Pelatihan')
...
@section('isi')
<div class="kepala">
  <h2>Antrean persetujuan pelatihan</h2>
  <p>Pendaftaran pelatihan dari bawahan dalam lingkup kewenangan Anda</p>
</div>
...
<div class="gulir">
  <table>
    <thead>
      <tr>
        <th>Pemohon</th><th>Kursus</th><th>Jadwal</th><th>Tindakan</th>
      </tr>
    </thead>
    <tbody>
      @forelse ($rows as $r)
        <tr>
          <td class="peg">{{ $r->employee_name }}<small>{{ $r->nrp }}</small></td>
          <td>{{ $r->course_title }}<small>{{ $r->batch_code }}</small></td>
          <td class="angka">{{ date('j M Y', strtotime($r->start_date)) }} – {{ date('j M Y', strtotime($r->end_date)) }}</td>
          <td>
            @if (auth()->user()->hasRole('auditor'))
              <span style="font-size:11px;color:var(--teks-lemah)">Hanya-baca</span>
            @else
              <div class="aksi">
                <form method="POST" action="{{ route('admin.lms-enrollment-approve', $r->id) }}">
                  @csrf
                  <button class="mini utama" type="submit">Setujui</button>
                </form>
                <form method="POST" action="{{ route('admin.lms-enrollment-reject', $r->id) }}" onsubmit="mintaAlasanTolak(this, event); return false;">
                  @csrf
      ...
      @empty
        <tr>
          <td colspan="4" class="kosong">Tidak ada pendaftaran yang menunggu keputusan.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
