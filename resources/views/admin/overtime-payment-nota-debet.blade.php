...
<body>
  ...
  <table class="data">
    <tr><td class="label">Perihal</td><td class="sep">:</td><td>Pembayaran Uang Lembur — {{ $batch->payer_scope === 'hc' ? 'Divisi '.$batch->division : $batch->office_name }}</td></tr>
  ...
  <table class="rincian">
    <thead>
      <tr><th>Pegawai</th><th>NRP</th><th>No. Rekening</th><th style="text-align:right">Jumlah Bersih</th></tr>
    </thead>
    <tbody>
      @foreach ($items as $i)
        <tr>
  ...
  <p class="catatan">
    ...
    berdasarkan batch pembayaran {{ $batch->spkl_number }}. Nominal kredit di atas adalah
    jumlah BERSIH (setelah dipotong pajak) — pemotongan pajak dicatat terpisah pada Jurnal
    Slip {{ $batch->spkl_number }}.
  </p>
</body>
</html>
