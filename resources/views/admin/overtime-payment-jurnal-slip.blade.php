...
<body>
  ...
  <div class="judul">
    <h2>JURNAL SLIP — PEMOTONGAN PAJAK LEMBUR</h2>
    <p>Nomor: {{ $batch->spkl_number }}</p>
  </div>

  <table class="data">
    <tr><td class="label">Perihal</td><td class="sep">:</td><td>Penyetoran Pajak Pembayaran Uang Lembur — {{ $batch->payer_scope === 'hc' ? 'Divisi '.$batch->division : $batch->office_name }}</td></tr>
    <tr><td class="label">Tanggal</td><td class="sep">:</td><td>{{ \Carbon\Carbon::parse($batch->created_at)->translatedFormat('l, d F Y') }}</td></tr>
    <tr><td class="label">Tarif Pajak</td><td class="sep">:</td><td>{{ $batch->tax_rate_percent }}%</td></tr>
    <tr><td class="label">Dasar Pengenaan (Bruto)</td><td class="sep">:</td><td>Rp{{ number_format($batch->total_gross_cents / 100, 0, ',', '.') }}</td></tr>
  </table>
  ...
  <table class="jurnal">
    ...
    <tbody>
      ...
      <tr>
        <td>{{ $batch->tax_account_code }} — {{ $batch->tax_account_name }}</td>
        <td class="angka">—</td>
        <td class="angka">Rp{{ number_format($batch->total_tax_cents / 100, 0, ',', '.') }}</td>
      </tr>
    </tbody>
  ...
  <p class="catatan">
    ...
    berdasarkan batch pembayaran {{ $batch->spkl_number }}. Nominal pajak ini digabung dengan
    nominal bersih pada Nota Debet {{ $batch->spkl_number }} akan sama dengan total bruto lembur.
  </p>
</body>
</html>
